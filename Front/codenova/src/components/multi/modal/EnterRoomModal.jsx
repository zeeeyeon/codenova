...
const EnterRoomModal = ({ isPrivate, roomTitle, roomLanguage, currentPeople,standardPeople,onClose, onEnter }) => {
          ...
          {isPrivate ? (
            <>
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") onEnter(roomCode);
                }}
                onFocus={(e) => (e.target.placeholder = "")} // 포커스 되면 placeholder 삭제
                onBlur={(e) => (e.target.placeholder = "방 코드 입력")} // 포커스 풀리면 다시 복구
                className="border rounded px-4 py-2 text-black w-[250px] mb-4 mx-auto text-center"
                placeholder="방 코드 입력"
              />
